  ...
  return (
    <section className="relative h-[85vh] min-h-[600px] flex items-center overflow-hidden bg-muted">
      <div className="absolute inset-0">
        <Image
          src={config.bgImage || '/images/hero.jpg'}
          alt="Hero"
          fill
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="max-w-xl">
          <p className="text-white/80 text-sm tracking-[0.3em] uppercase mb-4">Lahore&apos;s Finest</p>
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-light text-white leading-[1.1] mb-6">
            {config.headline || 'Style That Speaks You'}
          </h1>
          <p className="text-white/70 text-base sm:text-lg mb-10 max-w-md leading-relaxed">
            {config.subheadline || 'Discover curated fashion that blends modern elegance with timeless tradition.'}
          </p>
          <div className="flex flex-wrap gap-4">
            {config.ctaPrimary && (
              <Link
                href={config.ctaPrimary.link || '/shop'}
              ...
              >
                {config.ctaPrimary.text || 'Shop Collection'}
              </Link>
            )}
            {config.ctaSecondary && (
              <Link
                href={config.ctaSecondary.link || '/shop?category=traditional'}
              ...
              >
                {config.ctaSecondary.text || 'Traditional Wear'}
              </Link>
            )}
          </div>
        </div>
